...
echo ""
echo -e "${CYAN}You give it: vRealize workflows (PowerCLI support coming soon).${NC}"
...
print_narration "Import simple vRealize workflow (basic VM provisioning)"
wait_short
run_command "$OPS_CMD import --source vrealize --file ../examples/vrealize/simple-provision.workflow.xml"
wait_short

print_narration "Import environment-aware vRealize workflow (dev/prod branching)"
wait_short
run_command "$OPS_CMD import --source vrealize --file ../examples/vrealize/environment-branching.workflow.xml"
wait_short
...
echo -e "${BOLD}${GREEN}✓ 2. Import${NC} vRealize workflows (simple and NSX-complex)"
...
echo -e "  ${BOLD}Simple vRealize Workflows:${NC}"
echo -e "    • simple-provision.workflow.xml - Basic VM provisioning ${GREEN}(SUPPORTED)${NC}"
echo -e "    • environment-branching.workflow.xml - Dev/prod branching ${GREEN}(SUPPORTED)${NC}"
echo -e "  ${BOLD}Complex vRealize Workflows:${NC}"
