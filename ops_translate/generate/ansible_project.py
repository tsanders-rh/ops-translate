...
{chr(10).join(f"- {wf['name']}" for wf in workflows)}
...
{chr(10).join(f"- {tier.vmware_tier} → {tier.openshift_storage_class}" for tier in profile.storage_tiers) if profile.storage_tiers else "**Not configured** - storage tiers will use default storage class"}
