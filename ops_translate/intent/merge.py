...
def smart_merge(intents: list) -> dict:
    ...
    workflow_names = [i['data'].get('intent', {}).get('workflow_name') for i in intents if i['data'].get('intent', {}).get('workflow_name')]
    if workflow_names:
        merged['intent']['workflow_name'] = workflow_names[0]

    # Merge workload_type - use 'mixed' if different
    workload_types = set(i['data'].get('intent', {}).get('workload_type') for i in intents if i['data'].get('intent', {}).get('workload_type'))
    ...
    for intent in intents:
        ...
        for param_name, param_spec in inputs.items():
            if param_name not in all_inputs:
                ...
                if param_spec.get('description') and param_spec['description'] != existing.get('description'):
...
def _merge_governance(intents: list) -> dict:
    ...
    all_approvals = [i['data'].get('intent', {}).get('governance', {}).get('approval', {})
                     for i in intents if i['data'].get('intent', {}).get('governance', {}).get('approval')]
    ...
    all_quotas = [i['data'].get('intent', {}).get('governance', {}).get('quotas', {})
                  for i in intents if i['data'].get('intent', {}).get('governance', {}).get('quotas')]
...
def _merge_compute(intents: list) -> dict:
    ...
    all_compute = [i['data'].get('intent', {}).get('compute', {})
                   for i in intents if i['data'].get('intent', {}).get('compute')]
